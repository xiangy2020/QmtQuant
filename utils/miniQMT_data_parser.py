...
try:
    from env import xtdata as _xtdata_mod
...
def get_local_data(*args, **kwargs):
    """懒加载包装：调用时才访问 _xtdata_mod.get_local_data，避免模块导入时触发连接检查"""
    if _xtdata_mod is None:
        raise RuntimeError("xtdata 不可用，请检查 env 配置")
    return _xtdata_mod.get_local_data(*args, **kwargs)
                    ...
                    for i in range(record_count):
                        ...
                        if 'bidPrice' in tick_df.columns:
                            ...
                            if isinstance(bid_price_data, (list, np.ndarray)) and len(bid_price_data) >= 5:
                                bid_price_1 = round(float(bid_price_data[0]), 2) if bid_price_data[0] > 0 else '-'
                                bid_price_2 = round(float(bid_price_data[1]), 2) if bid_price_data[1] > 0 else '-'
                                bid_price_3 = round(float(bid_price_data[2]), 2) if bid_price_data[2] > 0 else '-'
                                bid_price_4 = round(float(bid_price_data[3]), 2) if bid_price_data[3] > 0 else '-'
                                bid_price_5 = round(float(bid_price_data[4]), 2) if bid_price_data[4] > 0 else '-'
                            elif isinstance(bid_price_data, (int, float)):
                                bid_price_1 = round(float(bid_price_data), 2) if bid_price_data > 0 else '-'
                        ...
                        if 'askPrice' in tick_df.columns:
                            ...
                            if isinstance(ask_price_data, (list, np.ndarray)) and len(ask_price_data) >= 5:
                                ask_price_1 = round(float(ask_price_data[0]), 2) if ask_price_data[0] > 0 else '-'
                                ask_price_2 = round(float(ask_price_data[1]), 2) if ask_price_data[1] > 0 else '-'
                                ask_price_3 = round(float(ask_price_data[2]), 2) if ask_price_data[2] > 0 else '-'
                                ask_price_4 = round(float(ask_price_data[3]), 2) if ask_price_data[3] > 0 else '-'
                                ask_price_5 = round(float(ask_price_data[4]), 2) if ask_price_data[4] > 0 else '-'
                            elif isinstance(ask_price_data, (int, float)):
                                ask_price_1 = round(float(ask_price_data), 2) if ask_price_data > 0 else '-'
                        ...
                        data.append({
                            '时间': time_str,
                            '最新价': round(float(last_price), 2) if last_price else 0.0,
                            '开盘价': round(float(open_price), 2) if open_price else 0.0,
                            '最高价': round(float(high_price), 2) if high_price else 0.0,
                            '最低价': round(float(low_price), 2) if low_price else 0.0,
                            '前收盘价': round(float(last_close), 2) if last_close else 0.0,
                            ...
                            '前结算': round(float(last_settlement_price), 2) if last_settlement_price else 0.0,
                            '买一价': bid_price_1,
                            '买一量': bid_vol_1,
                            '买二价': bid_price_2,
                            '买二量': bid_vol_2,
            ...
            if stock_code in kline_data:
                ...
                if isinstance(df, pd.DataFrame) and not df.empty:
                    ...
                    for i in range(record_count):
                        ...
                        if 'time' in df.columns:
                            ...
                            if isinstance(time_val, (int, float)):
                                try:
                                    ...
                                    time_str = dt.strftime('%Y-%m-%d') if period_type == '1d' else dt.strftime('%Y-%m-%d %H:%M:%S')
                                    if i == 0:
                            ...
                            try:
                                if isinstance(row_index, (int, float)):
                                    ...
                                    if row_index_int > 1000000000000:  # 毫秒时间戳（13位）
                                        dt = pd.to_datetime(row_index_int, unit='ms', utc=True).tz_convert('Asia/Shanghai').tz_localize(None)
                                    elif row_index_int > 1000000000:   # 秒时间戳（10位）
                                        dt = pd.to_datetime(row_index_int, unit='s', utc=True).tz_convert('Asia/Shanghai').tz_localize(None)
                                    elif len(str(row_index_int)) == 14:  # YYYYMMDDHHMMSS
                                        s = str(row_index_int)
                                        dt = datetime(int(s[0:4]), int(s[4:6]), int(s[6:8]),
                                                      int(s[8:10]), int(s[10:12]), int(s[12:14]))
                                    elif len(str(row_index_int)) == 8:   # YYYYMMDD
                                        s = str(row_index_int)
                                        dt = datetime(int(s[0:4]), int(s[4:6]), int(s[6:8]))
                                    else:
                                        dt = pd.to_datetime(str(row_index_int))
                                    time_str = dt.strftime('%Y-%m-%d') if period_type == '1d' else dt.strftime('%Y-%m-%d %H:%M:%S')
                                else:
                                    # pandas Timestamp 或字符串
                                    if hasattr(row_index, 'strftime'):
                                        time_str = row_index.strftime('%Y-%m-%d') if period_type == '1d' else row_index.strftime('%Y-%m-%d %H:%M:%S')
                                    else:
                        ...
                        data.append({
                            '时间': time_str,
                            '开盘价': round(float(open_val), 2) if open_val else 0.0,
                            '最高价': round(float(high_val), 2) if high_val else 0.0,
                            '最低价': round(float(low_val), 2) if low_val else 0.0,
                            '收盘价': round(float(close_val), 2) if close_val else 0.0,
                            '成交量': int(volume_val) if volume_val else 0,
                            '成交额': round(float(amount_val), 2) if amount_val else 0.0,
                            '今结算': round(float(settelement_price_val), 2) if settelement_price_val else 0.0,
                            '持仓量': int(open_interest_val) if open_interest_val else 0,
                            '前收价': round(float(pre_close_val), 2) if pre_close_val else 0.0,
